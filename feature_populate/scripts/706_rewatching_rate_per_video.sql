...
SELECT 706,
	...
	(
	SELECT SUM(get_if_rewatched(user_dropout.user_id, click_events.video_id))
    ...
    WHERE video_feature.feature_id = 704 AND video_feature.video_id = click_events.video_id
	),
    @current_date
FROM `moocdb`.click_events AS click_events
