...
DROP FUNCTION IF EXISTS `moocdb`.get_total_engagement_time;

CREATE FUNCTION get_total_engagement_time (USER_ID VARCHAR(50), VIDEO_ID VARCHAR(255))
    RETURNS FLOAT
    NOT DETERMINISTIC
BEGIN
...
SET feature_value = get_total_engagement_time(user_id, video_id)
